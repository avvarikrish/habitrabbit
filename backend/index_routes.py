# routes for adding location to index, recommending walking locations, and recommending sleep
...
from request_schemas import index_schema
...
def add_location():
    ...
    try:
        ...
        Schema(index_schema.add_location).validate(location)
...
def get_locations():
    ...
    try:
        ...
        Schema(index_schema.get_locations).validate(args)
        ...
        # get averages of past 7 days of scores
...
def get_sleep():
    ...
    try:
        # request args
        args = request.args.to_dict()
        Schema(index_schema.get_sleep).validate(args)
...
# parse weather object from OpenWeatherMap to get good time ranges to walk
def weather_parse(weather_response):
    final_weather_list = []
    previous_valid = -2
    one_complete = False
    
    # if current hour is in middle of day, mark hours 0 to current hour as invalid
    current_hour = datetime.datetime.now(TZ).hour
    if current_hour > 0:
        final_weather_list.append({'start': 0, 'end': current_hour, 'temp': '', 'description': '', 'valid': False})

    # go through all hours of weather
